...
Klaviyo.prototype.identify = function(identify) {
  ...
  // don't add $id if email only option is enforced to prevent some edge case dupe profile issues with Klaviyo API
  if (!this.options.enforceEmail) traitAliases.id = '$id';
  var traits = identify.traits(traitAliases);
  if (this.options.enforceEmail && traits.$id) return;
  // if you don't enforce but you are missing either id or email, noop
  if (!this.options.enforceEmail && !traits.$id && !traits.$email) return;
